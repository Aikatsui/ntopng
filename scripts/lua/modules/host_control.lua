...
-- This is the minimum control_group id which will be used to create new hosts_control
hosts_control.MIN_ASSIGNED_CONTROL_GROUP_ID = 0
...
local function _get_control_group_ids_key()
   local key = string.format("%s.control_group_ids", _get_hosts_control_prefix_key())
...
local function _get_control_group_lock_key()
   local key = string.format("ntopng.cache.hosts_control.control_group_lock")
...
local function _get_control_group_details_key(control_group_id)
   ...
   local key = string.format("%s.control_group_id_%d.details", _get_hosts_control_prefix_key(), control_group_id)
...
local function _assign_control_group_id()
   -- OVERRIDE
   -- To stay consistent with the old implementation hosts_control_nedge.lua
   ...
   local next_control_group_id = hosts_control.MIN_ASSIGNED_CONTROL_GROUP_ID
...
function hosts_control.add_control_group(name, members)
   local locked = _lock()
   ...
   if locked then
	 ...
	 local same_name_control_group = hosts_control.get_control_group_by_name(name)
	 if same_name_control_group then
	    checks_ok = false
	 end

	 -- Check if members are valid
	 if check_ok and not hosts_control.are_valid_members(members) then
	    checks_ok = false
	 end
...
function hosts_control.edit_control_group(control_group_id, new_name, new_members)
   local ret = false
   local locked = _lock()
   ...
   if locked then
      ...
      local cur_details = hosts_control.get_control_group(control_group_id)
	 ...
	 local same_name_control_group = hosts_control.get_control_group_by_name(new_name)
	 if same_name_control_group and same_name_control_group.control_group_id ~= control_group_id then
	    checks_ok = false
	 end

	 -- Check if members are valid
	 if checks_ok and not hosts_control.are_valid_members(new_members) then
	    checks_ok = false
	 end
...
function hosts_control.disable_control_group_flow_alert(control_group_id, alert_key)
   local ret = false
   local locked = _lock()
...
--@brief Marks an alert as disabled for a given control group identified with `control_group_id`
--@return True, if alert is disabled with success, false otherwise
function hosts_control.enable_control_group_flow_alert(control_group_id, alert_key)
   local ret = false
   local locked = _lock()
...
function hosts_control.delete_control_group(control_group_id)
   local ret = false
   local locked = _lock()
...
-- @brief Returns all the defined hosts_control. Hosts_Control are returned in a lua table with control_group ids as keys
function hosts_control.get_all_hosts_control()
   local cur_control_group_ids = _get_assigned_control_group_ids()
   local res = {}

   for _, control_group_id in pairs(cur_control_group_ids) do
      local control_group_details = hosts_control.get_control_group(control_group_id)
...
function hosts_control.get_num_hosts_control()
   local cur_control_group_ids = _get_assigned_control_group_ids()
...
function hosts_control.get_control_group(control_group_id, recipient_details)
...
function hosts_control.cleanup()
   ...
   local cur_control_group_ids = _get_assigned_control_group_ids()
   for _, control_group_id in pairs(cur_control_group_ids) do
      hosts_control.delete_control_group(control_group_id)
   end
...
function hosts_control.is_valid_member(member)
...
function hosts_control.are_valid_members(members)
   for _, member in pairs(members) do
      if not hosts_control.is_valid_member(member) then
	 return false
      end
   end
...
function hosts_control.get_control_group_by_name(name)
   local cur_control_group_ids = _get_assigned_control_group_ids()
